...
function ViewFeedback() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/viewfeedback')
...
const deletefeedbackrecord = (id) => {
  axios.delete(`http://localhost:4000/deletefeedbackrecord/${id}`)
    .then(res => {
      console.log(res);
...
const filteredData = user.filter((data) => {
  return (
    ...
    data.feedback.toLowerCase().includes(searchQuery.toLowerCase())
                    ...
                    <tbody>
                      {filteredUser.map((data, index) => (
                        <tr key={index}>
                          ...
                          <td>{data.feedback}</td>
                          <td>
                            <button className="btn btn-danger text-light" onClick={()=> deletefeedbackrecord(data._id)}>Delete</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
